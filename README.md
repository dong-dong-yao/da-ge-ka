...
- 可选桌面宠物：开启后角色常驻桌面角落（默认右下角），透明置顶、鼠标穿透不挡操作；它会跟随鼠标指针移动手臂，在按下鼠标左键或右键时给出按压反馈。键盘右手按左、中、右三个键区切换按压姿势，肩部固定，下掌短促下压并回弹；短按也会显示，长按保持触键，松开后恢复原待机抬手。白熊身体、桌面、键盘和鼠标沿用 `Assets/DesktopPet/white-bear-typing.jpg` 原图；三张用户提供的透明 PNG 存放于 `Assets/DesktopPet/KeyboardPress/`，只提取右臂使用，并排除动作线。托盘菜单可切换「调整宠物位置」模式拖动换位。默认关闭；在设置页打开开关后必须点击「保存设置」才会生效，也可从托盘菜单直接开启。
