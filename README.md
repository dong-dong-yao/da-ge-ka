...
- 可选桌面宠物：开启后角色常驻桌面角落（默认右下角），透明置顶、鼠标穿透不挡操作；它会跟随鼠标指针移动手臂，在按下鼠标左键或右键时给出按压反馈，并根据当前按键位置移动键盘手。停止输入后会平滑恢复待机。当前白熊直接使用 `Assets/DesktopPet/white-bear-typing.jpg` 原图，运行时仅做边缘连通背景透明化和程序化动作反馈，不生成或替换角色画面。托盘菜单可切换「调整宠物位置」模式拖动换位。默认关闭；在设置页打开开关后必须点击「保存设置」才会生效，也可从托盘菜单直接开启。
- 桌面宠物通过全局低级键盘和鼠标钩子获取瞬时输入状态。键盘事件仅在内存中瞬时读取 Windows virtual-key identity，只用于确定键盘手的位置；程序不会把它转换为输入文字，也不会把该值写入日志、保存、上传或传输。鼠标事件仅在内存中保留当前指针坐标及左右键按下状态，用于手臂跟随与点击反馈。程序本身不联网。若安全软件拦截钩子安装，宠物会静默降级为静态显示，不影响其他功能。
...
4. 打开 Windows 记事本并输入几组分布在键盘不同区域的按键，确认键盘手会移动到不同位置，同时记事本输入不受影响。
5. 在托盘菜单关闭「调整宠物位置」，确认宠物处于鼠标穿透状态，能够点击它后方的窗口；再开启该模式，确认可以拖动宠物换位。
6. 若宠物只有静态画面，检查安全软件是否拦截了全局钩子。不要据此判断提醒、设置或托盘等其他功能失效。
